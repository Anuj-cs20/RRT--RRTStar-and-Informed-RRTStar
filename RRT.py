...
# addChild function to add the child to the nearest node only if it improves the cost of the path to the child
			...
			if costTemp < tempNode.cost:
				...
				for sibling in self.nearestNode.children:
					if sibling != tempNode and not self.isInObstacle(sibling, tempNode):
						costSib = sibling.cost + self.distance(sibling, tempNode)
						if costSib < tempNode.cost:
							tempNode.parent = sibling
							tempNode.cost = costSib
							sibling.children.append(tempNode)
							self.rewire(tempNode)
	...
	def rewire(self, node):
		for child in node.children:
			...
			if new_cost < child.cost and not self.isInObstacle(node, child):
				child.parent = node
				child.cost = new_cost
				self.rewire(child)
